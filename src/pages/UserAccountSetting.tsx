...
import { Camera, Shield, CreditCard, ArrowLeft, FileText, ShieldOff } from "lucide-react";
...
const UserAccountSetting = () => {
  ...
  const isDisabled =
    ...
    !(
      formData.image &&
      (formData.image instanceof File || typeof formData.image === "string")
    );
  ...
  useEffect(() => {
    if (authVar?.user?.phoneNumber) {
      setFormData((prev) => ({
        ...prev,
        firstName: authVar?.user?.firstName || "",
        lastName: authVar?.user?.lastName || "",
  ...
  const handleEmailOtpverfy = () => {
    dispatch(
      userEmailVerifyOtp({ email: formData?.email, otp: otp }, setEmailModal)
    );
  };
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div
        className={`${
          isMobile ? "p-4 pb-20 pt-4" : "container mx-auto px-6 py-8"
      ...
      >
        ...
        {authVar?.user?.isAadharVerified && !authVar?.user?.isEmailVerified && (
          <Card className="px-4 py-2 bg-red-100 border border-red-300 rounded-lg max-w-md mx-auto">
        ...
        {authVar?.user?.isAadharVerified && authVar?.user?.isEmailVerified && authVar?.user?.status === 'pending' && (
          <Card className="px-4 py-2 bg-yellow-100 border border-yellow-300 rounded-lg max-w-md mx-auto">
            <div className="flex items-center gap-2">
                ...
                >
                  {({ getRootProps, getInputProps }) => (
                    <div
                      ...
                      } rounded-full bg-muted flex items-center justify-center cursor-pointer overflow-hidden`}
                    >
                      <input
                        ...
                        <div className="flex justify-end gap-2">
                          {!otpSent ? (
                            <Button
                              ...
                              disabled={authVar?.status === "loading"}
                            >
                              {authVar?.status === "loading"
                                ? "Sending..."
                                : "Send OTP"}
                            </Button>
                          ) : (
                            <Button
                              onClick={handleEmailOtpverfy}
                              disabled={authVar?.status === "loading"}
                            >
                              {authVar?.status === "loading"
                                ? "Verifying..."
      ...
      {modal && (
        <div className="modal bg-white w-full h-screen fixed top-0 right-0 z-50 flex items-center justify-center">
          <div>
